...
$\delta_{t} \overset{\cdot}{=} R_{t+1} - \bar{R}_t + \hat{v}(S_{t+1}, \mathbf{w}_{t}) - \hat{v}(S_{t}, \mathbf{w}_{t})$
...
IS ratio $\rho_t$. This is because we're 
estimating the value for $(S_t, A_t)$ following $\pi$. This means, 
we're at $A_t$ already, and we care how the ratios of the subsequent
actions, i.e. $A_{t+1:t+n}$ are taken not how we got to $A_t$.

For Expected Sarsa and Q-Learning we also do not need $\rho_{t+1}$ because:
* for Q-Learning case, the next action is deterministic in action 
(we select the greedy action), hence no correction is needed
* for Expected Sarsa the boostrapped part is the expectation wrt $\pi$

So, for Q-Learning and Expected Sarsa one-step case, the control algorithm 
is the same as the on-policy. However, we must correct the bootstrapped
value for Sarsa since we use the sample estimate which was drawn from the 
behavioral policy.
...
therefore the appropriate IS at $t$, namely $\rho_t$ adjustment is  
needed for the sampled actions $A_t \sim b(\cdot|S_t)$, in order to 
obtain the sample-estimate $v(S_t = s)$. However, 
for $q(s, a) = \mathbb{E}_{P(\cdot|s, a)}[r + \gamma v_{\pi}(s')]
= \mathbb{E}_{P(\cdot|s, a)}[r + \gamma  \mathbb{E}_{\pi(\cdot|s'), P}[q_{\pi}(s', a')]]$ 
the sample estimate $q(S_t = s, A_t = a)$ is already starting our analysis 
at $A_t$,  i.e. there is no $\mathbb{E}_{\pi(\cdot|s)}$ expectation 
that depends on the $a$ under consideration. 

But the analysis gets a bit more complicated for the next step weights 
$\rho_{t+1}$:

* For Q-Learning, the boostrapped term is $\gamma\, \max_a q(S_{t+1}, a)$, hence 
deterministic. No next-step IS correction is needed
* For Expected Sarsa, the bootstrapped term is the analytic expectation wrt $\pi$, so 
no correction is needed here either.
* For Sarsa however, our next step estimate is sample-based, i.e. 
$(S_{t+1}, A_{t+1})$. It is necessary therefore to compute
the correction for the next step $\rho_{t+1}$

#### Multi-Step
In the multistep case the one-step algorithms are extended as follows:

###### Target

__Prediction__
* $G_{t:t+n} = R_{t+1} + \gamma R_{t+2} + ... + \gamma^{n-1}R_{t+n} + \gamma^{n}\hat{v}(S_{t+n}, \mathbf{w}_{t+n - 1})$ (episodic)
* $G_{t:t+n} = R_{t+1} - \bar{R}_{t} + R_{t+2} - \bar{R}_{t+1} + ... + R_{t+n} - \bar{R}_{t+n-1} + \hat{v}(S_{t+n}, \mathbf{w}_{t+n - 1})$ (continuing)
...
we do not IS weigh the $(S_t, A_t)$ sample, since we're starting our estimation
at $q(S_t, A_t)$, but we do weigh the subsequent samples as their choice depends 
on the appropriate expectation. This reasoning makes sense for the 
tabular case, where each $(S_t, A_t)$ update is independent of others. 
But for the function approximation approach, this assumption does not hold - 
because certain state-action pairs affect the generalization of others. Proper
resolution has not been found yet. 

For the multi-step prediction case $\hat{v}$, the IS weights are synchronized 
with the actions which generated sampled rewards, whereas for $\hat{q}$ they 
are shifted one step forward, i.e. synchronized the next rewards, 
i.e. $R_{t+1:t+n}$ and the last $\hat{q}(S_{t+n}, A_{t+n})$.
...
Any approximation of it $\hat{v}_{\pi} = v_{\mathbf{w}, \pi}$ will yield 
an error which is called _Bellman Error_ (BE) $\bar{\delta}$ at state $s$. 
This error could be used as a measure of how far off our approximation
is from the true value function, and it is defined as:
...
We see that __BE is the _expected_ TD error under $\pi$__. The vector of 
...
It is usually not possible to reduce $\overline{BE}(\mathbf{w}) = 0$ 
where we would get $v_{\pi} = v_{\mathbf{w}}$ (i.e. the unique solution to the 
Bellman Value Equation), but for linear approximation there is a 
$\mathbf{w}$ for which $\overline{BE}$ is minimized.
...
$$
  \begin{align*}
    \overline{TDE}(\mathbf{w}) &= \mathbb{E}_{\mu_{\pi}}[\mathbb{E}_{\pi}[\delta_{t}^2 | S_t]] \\
    &= \sum_{s}\mu_{\pi}(s)\mathbb{E}_{\pi}[\delta_{t}^2 | S_t = s, A_t \sim \pi] \\
    &= \sum_{s}\mu_{b}(s)\mathbb{E}_{b}[\rho_t \delta_{t}^2 | S_t = s, A_t \sim b] \\
    &= \mathbb{E}_{\mu_{b}}[\mathbb{E}_{b}[\rho_t \delta_{t}^2 | S_t]] \\
...
$$
\begin{align*}
\overline{BE} &= \sum_{s}\mu(s)\bar{\delta}_{\mathbf{w}}(s)^2 \\
              &= \mathbb{E}_{\mu_{\pi}}[(\mathbb{E}_{\pi}[\delta_{t,  \mathbf{w}} | S_{t}])^2]
\end{align*}
$$
...
###### Discussion for the strange distribution of $\rho_t$ above
...
$\mathbb{E}_{b}[v_{\pi}] = v_{\pi} = \hat{v}$. Note the footnote [1] in Ch. 11.5 under 
...
from $b$ $-$ which is not clear whether corresponds to the original definition 
or not. We can make the case similar to the control case where we don't care how we 
...
$$
\begin{align*}
  \mathbf{w}_{t+1} &= \mathbf{w}_{t} + \alpha [\mathbb{E}_{b}[\rho_t (R_{t+1} + \gamma \hat{v}(S_{t+1}, \mathbf{w}_t)] - \hat{v}(S_{t}, \mathbf{w}_t)][\nabla_{\mathbf{w}}\hat{v}(S_{t}, \mathbf{w}_t) - \gamma \mathbb{E}_{b}[\rho_t\nabla_{\mathbf{w}}\hat{v}(S_{t+1}, \mathbf{w}_t)]]
\end{align*}
$$
...
minor difference wrt the treatment of $\rho_t$ as we discussed). 
This is naive because  the expectation wrt $S_{t+1}$ is multiplied together. 
Since we're using  the same sample $S_{t+1}$ we get a _biased estimate_ of 
the expectation. To  obtain an un-biased estimate of the expectation, 
we would need two independently  sampled $S_{t+1}$'s. But we only get one 
sample in the interaction with the  environment. There are two ways to de-bias
this expectation:

* If the environment is deterministic (i.e. $S_{t+1, k} = S_{t+1, j})$, the
estimate is un-biased
