"""
    Taken from:
    https://github.com/konantian/Dyna-Maze-Game/blob/master/Codes/rl_glue.py
"""
            ...
            for col in range(6):
                ...
                if self.maze[row][col] == 1:
                    ...
                    pygame.draw.rect(self.surface,
                                     pygame.Color(self.color[value]), grid)

    def showChar(self):
        pygame.font.init()
        myfont = pygame.font.SysFont('Comic Sans MS', 45)
        start = myfont.render('S', False, (0, 0, 0))
        self.surface.blit(start, (self.start[0] * (self.w + self.margin) + 20,
                                  self.start[1] * (self.w + self.margin) + 20))

        goal = myfont.render('G', False, (0, 0, 0))
        self.surface.blit(goal, (self.goal[0] * (self.w + self.margin) + 20,
                                 self.goal[1] * (self.w + self.margin) + 20))
...
class BaseEnvironment:
    ...
    @abstractmethod
    def env_init(self, **kwargs):
        """
        Initialize environment variables.
        """

    @abstractmethod
    def env_start(self, **kwargs):
        """
